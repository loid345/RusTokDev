...
- `apps/server` — **голая Loco.rs-платформа**: хост-оболочка, которая целиком строится на Loco.rs (boot, config, auth, migrations, workers, tasks, mailer, storage), подключает admin/storefront UI-клиентов к единому API и регистрирует поверх этого каркаса доменные модули (`src/modules/mod.rs`). Собственной бизнес-логики не содержит.
...
1. `apps/server/src/app.rs` — boot, routes, middleware (Loco lifecycle).
2. `apps/server/src/modules/mod.rs` — `build_registry()` — точка подключения доменных модулей.
3. `apps/server/src/middleware/tenant.rs` — tenant resolution и кэш.
4. `apps/server/src/controllers/*` + `apps/server/src/graphql/*` — API слой.
5. `crates/rustok-core` — инфраструктурные интерфейсы.
6. `crates/rustok-*/src/services` — бизнес-логика модулей.
