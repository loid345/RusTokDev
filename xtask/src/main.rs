...
fn load_manifest() -> Result<Manifest> {
    let content = fs::read_to_string("modules.toml").context("Failed to read modules.toml")?;
    let manifest: Manifest = toml::from_str(&content).context("Failed to parse modules.toml")?;
        ...
        match spec.source.as_str() {
            ...
            "crates-io" => {
                if spec.version.is_none() {
                    anyhow::bail!(
                        "Module '{}' has source='crates-io' but no version specified",
                        slug
                    );
                }
            }
