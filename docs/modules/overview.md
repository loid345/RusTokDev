...
- `adminNavItems` (или эквивалентный контракт admin-навигации).
...
- для admin-runtime: `apps/admin` и `apps/next-admin`;
- для storefront-runtime: `apps/storefront` и `apps/next-frontend`.
...
| `blog` | `crates/rustok-blog/ui/admin` | `crates/rustok-blog/ui/frontend` | ⚠️ Partial | Выделить/стандартизировать явные `adminNavItems` и `frontendNavItems` export points под единый контракт для Leptos+Next runtimes. |
