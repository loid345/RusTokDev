...
## 7. Readiness-check для внедрения Tiptap / Page Builder (blog/forum/pages/content)
...
### 7.1 Что уже готово (можно использовать для rollout)

- **Единый backend-контракт rich-text уже реализован в core**:
  `markdown` + `rt_json_v1`, нормализация legacy alias `rt_json`, обязательный sanitize/validation на сервере через `prepare_content_payload(...)` и `validate_and_sanitize_rt_json(...)`.
- **Blog/Forum/Pages сервисы принимают `content_json` + format и отдают format-aware response**:
  `body_format/content_format` + `content_json` (для `markdown` остаётся plaintext fallback).
- **Есть tenant-scoped migration job** для перевода legacy markdown в `rt_json_v1`:
  `apps/server/src/bin/migrate_legacy_richtext.rs` (idempotent/checkpoint/retry).
- **Pages BlockService уже обеспечивает schema-first валидацию блоков и URL/embed/HTML sanitize policy**, что технически совместимо с Page Builder UX.
...
### 7.2 Ключевые зазоры (что блокирует production-ready внедрение)

- **Admin/runtime интеграция UI-компонентов неполная**:
  компоненты Tiptap/Page Builder находятся в отдельном пакете, но не подключены в реальные `apps/next-admin`/`apps/admin` потоки как обязательный production UX.
- **Forum/Blog productionization остаётся незавершённой** по собственным планам модулей:
  часть RBAC/runtime hardening и operational задач ещё в backlog.
- **Pages implementation-plan остаётся high-level** (Phase 2/3 в статусе planned), нет зафиксированного rollout-checklist именно для массового Page Builder внедрения.
...
### 7.3 Практический verdict по готовности

- **Backend-контракт и data-layer: готовность высокая** — база для Tiptap/Page Builder уже есть.
- **Frontend adoption readiness: средняя** — есть reusable-компоненты, но нужна интеграция в боевые админ-приложения.
- **Production rollout readiness: средняя/ниже средней** — до включения фичи по умолчанию нужны закрытие backlog по hardening + расширение integration/e2e срезов.

### 7.4 Минимальный go-live checklist (рекомендуемый)
...
2. Зафиксировать migration runbook tenant-by-tenant на основе `migrate_legacy_richtext` + rollback policy.
3. Довести CI-ready интеграционные тесты blog/forum/pages для create/update/read в `rt_json_v1` режиме.
4. Закрыть P0/P1 hardening задачи из implementation-plan (минимум RBAC enforcement + observability/release-gate).
