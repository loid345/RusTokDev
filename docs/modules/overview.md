...
### 8.1 Что уже готово (можно использовать для поэтапного запуска)
...
### 8.2 Ключевые зазоры (что блокирует внедрение уровня production)
...
### 8.3 Практический вывод по готовности
...
### 8.4 Минимальный чеклист перед запуском (рекомендуемый)
...
### 8.5 Пошаговый план выполнения (чтобы «делать по списку»)

Используйте этот список как рабочий трекер подготовки. Рекомендуемый порядок: сначала контракт и безопасность, затем UX-интеграция, затем rollout.

- [ ] **Шаг 1 — зафиксировать целевой контракт редактора в admin UI**
  - Подтвердить единый payload: `format=rt_json_v1` + `content_json`, fallback только для legacy.
  - Для всех форм Blog/Forum/Pages зафиксировать единые правила валидации клиентских ошибок (до отправки) и серверных ошибок (после sanitize).
- [ ] **Шаг 2 — интегрировать редакторы в production-маршруты админки**
  - В `apps/next-admin` подключить `RtJsonEditor`/`PageBuilder`/`ForumReplyEditor` в реальные CRUD-flow.
  - Синхронно определить parity-план для `apps/admin` (Leptos), чтобы не разъезжались UI-контракты.
- [ ] **Шаг 3 — ввести feature flags и сценарий включения**
  - Добавить rollout-флаги: per-tenant/per-module/per-form.
  - Подготовить матрицу включения: internal tenants → pilot tenants → default-on.
- [ ] **Шаг 4 — подготовить миграцию legacy markdown → rt_json_v1**
  - Для каждого tenant выполнить dry-run через `migrate_legacy_richtext` и сохранить отчёт (processed/succeeded/failed/skipped).
  - До боевого запуска подтвердить rollback policy (backup scope + точка восстановления).
- [ ] **Шаг 5 — закрыть release-gate по качеству**
  - Довести integration/e2e сценарии Blog/Forum/Pages до CI-ready (create/update/read/publish/moderation).
  - Проверить RBAC enforcement для editor/moderator/admin ролей на новых маршрутах.
  - Добавить observability минимум: error-rate, sanitize-failures, publish latency, migration metrics.
- [ ] **Шаг 6 — выполнить pre-production smoke и post-rollout контроль**
  - Smoke: create/update/read, preview/publish, index/reindex, canonical URL, rendering parity в storefront.
  - Post-rollout: мониторинг 24–72 часа, fallback-процедура, фиксация итогов и обновление implementation-plan/docs.
