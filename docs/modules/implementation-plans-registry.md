...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `84%` | `agent` | `2026-05-24T21:05:00Z` | `в C1 завершена декомпозиция sitemaps на 3 дочерних runtime-модуля: index-generation, submission-adapters, submission-aggregation; public SeoSitemapStatusRecord не изменён` | Зафиксировать provider seam как отдельный runtime strategy/factory object и закрыть C1 verification matrix (`cargo test -p rustok-seo --lib sitemaps`) | `provider-specific adapters блокируются отсутствием tenant-secret/rotation contract` | `cargo test -p rustok-seo --lib sitemaps` |
