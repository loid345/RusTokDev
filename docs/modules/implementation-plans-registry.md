...
| `rustok-tenant` | `rustok-tenant` | `crates/rustok-tenant/docs/implementation-plan.md` | `in_progress` | `85%` | `agent` | `2026-05-21T12:00:00Z` | `добавлены resolver invariants integration tests (header/host/subdomain + disabled/not-found) и зафиксирован provisioning/deprovisioning + invalidation contract` | Закрыть оставшийся contract-sync пункт в `Contract stability` и обновить verification notes | `-` | `cargo test -p rustok-tenant --lib` |
