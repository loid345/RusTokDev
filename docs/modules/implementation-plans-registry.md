...
| `rustok-commerce` | `rustok-commerce` | `crates/rustok-commerce/docs/implementation-plan.md` | `in_progress` | `46%` | `agent` | `2026-05-30T09:58:38Z` | `Phase 10.4 claim decision hardening: graphql_runtime_parity_test stale refund/payment/tax helpers aligned with current contracts, graphql_runtime_parity_test target and filtered package-wide post-order run pass` | Расширить post-order operator UX и связать exchange/claim return decisions с order-change apply/cancel lifecycle без host-owned logic | default server OpenAPI test блокируется существующими compile errors вне commerce; commerce graphql_runtime_parity target, package-wide filtered post-order run, targeted bridge/checkout tests и commerce check проходят | `cargo check -p rustok-commerce`; `cargo test -p rustok-commerce --test graphql_runtime_parity_test`; `cargo test -p rustok-commerce commerce_post_order_decision_creates_return_bound_claim_change -- --exact`; `cargo test -p rustok-commerce --test order_returns_bridge_test`; `cargo test -p rustok-commerce --test checkout_service_test complete_checkout_rejects_reentry_for_checking_out_cart_without_artifacts -- --exact` |
