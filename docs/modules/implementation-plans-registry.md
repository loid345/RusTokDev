...
| `rustok-seo` | `rustok-seo` | `crates/rustok-seo/docs/implementation-plan.md` | `in_progress` | `88%` | `agent` | `2026-05-28T23:58:00Z` | `закрыт D1 contract freeze: зафиксированы D1..D9 backlog, additive-only v1 compatibility policy и rollout flags для events/outbox/index/REST parity/Next runtime` | Стартовать D2: typed SEO events + outbox foundation (idempotency keys, duplicate-emission guard в bulk paths, migration skeleton) | локально blocked verification: `cargo` отсутствует в PATH; для D2/D3 нужен cross-owner sign-off `seo + outbox + index` | `cargo check -p rustok-seo --tests --config profile.dev.debug=0` |
