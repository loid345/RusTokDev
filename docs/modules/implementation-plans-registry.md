...
| `rustok-inventory` | `rustok-inventory` | `crates/rustok-inventory/docs/implementation-plan.md` | `in_progress` | `55%` | `agent` | `2026-06-06T00:00:00Z` | `Wave 5 inventory admin boundary: backend AdminInventoryReadService/DTO + native read server functions live; set/adjust/reserve quantity write endpoints use inventory-owned native facade and admin UI controls with typed InventoryQuantityWriteResult / InventoryReservationWriteResult plus backend inStock derivation/wire-shape evidence; native admin read-side uses reservation-aware inventory-level available quantity with legacy variant fallback; transitional commerce GraphQL adapter is package-private read-only fallback with removal criteria and boundary coverage` | Продолжить следующий remaining inventory mutation/write parity slice после source-level guard для commerce public-channel availability callers | Первый CI migration-smoke прогон ещё нужно наблюдать; remaining write mutation parity не завершён | `cargo test -p rustok-inventory --lib`; `cargo test -p rustok-inventory-admin --test boundary` |
