...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #35: admin submit/transport error message policy moved into Leptos-free `RegionAdminSubmitErrorLabels`, `RegionAdminTransportErrorLabels` and core helpers; slice #34 moved save-success selected/form/refresh/route-update outcome mapping into core; fast guardrail `scripts/verify/verify-region-admin-boundary.mjs`; storefront keeps native/GraphQL transport split) |
