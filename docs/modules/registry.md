...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `docs_boundary` | `crates/rustok-commerce/docs/implementation-plan.md` (slice 10.4: admin REST/GraphQL return decision-tree transport plus native-first module-owned post-order order-change operator over `OrderService` with GraphQL kept in parallel) |
