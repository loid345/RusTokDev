...
| `commerce` | admin + storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-commerce/docs/implementation-plan.md` (admin FFA core module split + FBA-readiness gate: crate root only wires/re-exports `ui::CommerceAdmin`; framework-agnostic `admin/src/core/` is split into common/shipping_profile/promotion/presentation/order_change submodules for form/draft policy, optional filters, commands, badge/display view-models and resolution mapping; native-first post-order operator keeps module-owned transport + GraphQL fallback) |
