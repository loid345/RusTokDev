...
| `cart` | storefront | `in_progress` | `in_progress` | `core_transport_ui` | `crates/rustok-cart/docs/implementation-plan.md` (slice: storefront `core/request.rs` owns fetch/decrement/remove request objects; GraphQL fallback dispatches the core decrement command without re-deriving quantity policy in API layer) |
