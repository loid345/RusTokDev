...
| `region` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-region/docs/implementation-plan.md` (slice #36: selected-region route/query writer operation moved into Leptos-free `RegionAdminRouteQueryWrite` so push/replace/clear host update policy is core-tested; slice #35 moved submit/transport error message policy into core; fast guardrail `scripts/verify/verify-region-admin-boundary.mjs`; storefront keeps native/GraphQL transport split) |
