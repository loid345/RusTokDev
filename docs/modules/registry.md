...
| `workflow` | admin | `in_progress` | `in_progress` | `crates/rustok-workflow/docs/implementation-plan.md` (slice: admin core helpers + thin `transport/` facade around GraphQL adapter; Leptos layer потребляет view-models и facade, внешний GraphQL contract не изменялся) |
