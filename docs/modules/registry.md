...
| `blog` | admin + storefront | `in_progress` | `not_started` | `core_transport_ui` | `crates/rustok-blog/docs/implementation-plan.md` (slice #76: admin save-command preparation (`prepare_blog_post_save_command`) moved into `admin/src/core.rs` with unit-test evidence for validation rejection plus create/update operation selection; Leptos adapter delegates validation/outcome/busy-key policy to core while dispatching through the module-owned transport facade; previous slice added explicit admin/storefront `ui/leptos.rs` render adapters and storefront adapters consume core-owned fetch request) |
