...
| `rustok-ai` | AI host/orchestrator capability with large operator/admin UI surfaces for Leptos and Next.js hosts; Leptos admin FFA first core/transport/ui slice is tracked in `crates/rustok-ai/docs/implementation-plan.md` with native adapter `admin/src/transport/native_server_adapter.rs` and guardrail `scripts/verify/verify-ai-admin-boundary.mjs` |
