...
- [ ] Добавить `PlatformCompositionBuildService` (server/control-plane слой).
- [ ] Перенести `validate manifest -> CAS update platform_state -> build enqueue` в один transaction boundary.
- [ ] Гарантировать одинаковый `manifest_ref = platform_state:<revision>` для GraphQL и Leptos SSR.
- [ ] Перевести admin SSR path с raw SQL helper на shared service/adapter.
- [ ] Удалить/ограничить legacy helper `save_manifest_and_enqueue_build()` после перевода.
...
- [ ] Оставить `ModuleLifecycleService::toggle_module_with_actor()` как единственный production entrypoint.
- [ ] Переподключить `toggle_module_native()` в `apps/admin` к canonical lifecycle service.
- [ ] Перевести/закрыть прямой model-level toggle bypass в `tenant_modules`.
- [ ] Добавить repo-guard (test/script) против production вызовов bypass API.
...
- [ ] Единственный production entrypoint: `toggle_module_with_actor()`.
- [ ] Все admin/runtime surfaces используют только canonical lifecycle service.
- [ ] Bypass API в model-layer недоступен из production кода.

**Deliverables.**

- [ ] Явный “unsafe for migrations/tests only” contract для low-level toggle API.
- [ ] Repo-guard (test/script), блокирующий production references bypass API.
