...
### Актуализация 2026-05-30 (итерация 82)

- Снят текущий format-blocker strict-runner: удалены конфликтующие duplicate module roots `crates/rustok-cart/storefront/src/core.rs` и `crates/rustok-cart/storefront/src/transport.rs`, а workspace выровнен через `cargo fmt --all`; после этого `cargo fmt --all -- --check` проходит.
- Снят migration-blocker минимального набора: `order_changes` migration больше не генерирует SQLite-incompatible inline non-unique indexes, а `order_returns` resolution migration больше не использует multi-action `ALTER TABLE`; `cargo test -p migration` проходит, включая ecommerce schema smoke.
- Полный strict minimal runner пока не закрыт: `RUSTOK_VERIFY_STEP_TIMEOUT=420s scripts/verify/run-control-plane-remediation-minimal.sh` дошёл до `cargo test -p rustok-server module_lifecycle`, но остановился по timeout во время холодной перекомпиляции server/admin dependency graph; отдельный `timeout 360s cargo test -p rustok-server module_lifecycle` также завершился `124` на compile phase. Поэтому release-gate чекбокс остаётся `[ ]`, Batch-2 пункт остаётся `[~]`, а план ещё не завершён до зелёного server test window и последующего `--fail-on-open`.

### Актуализация 2026-05-30 (итерация 83)
