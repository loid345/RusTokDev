...
- [~] journal (`module_operations`) пишется всегда при смене effective state (добавлены server integration tests `successful_toggle_writes_done_module_operation`, `hook_failure_rolls_back_state` (failed journal), `successful_toggle_with_actor_persists_requested_by`, проверки что idempotent retry не создаёт duplicate journal rows и что pre-validation failures (`MissingDependencies`/`HasDependents`) не создают лишние journal rows; остаётся parity-покрытие GraphQL/Leptos SSR);
