...
- [~] Убрать drift: GraphQL, BuildService и Leptos SSR используют один canonical hashing contract (Leptos SSR path canonicalizes JSON snapshot перед SHA-256; SSR unit tests покрывают SHA-256 format/stability/change detection и fixed vector; server integration test `successful_enqueue_keeps_hash_parity_between_snapshot_and_build` дополнительно фиксирует parity `snapshot.manifest_hash == build.manifest_hash`; остаётся закрыть cross-surface parity test на один и тот же manifest → один hash/ref/snapshot для GraphQL+Leptos SSR end-to-end).
