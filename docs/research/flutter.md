...
- **Следующая точка контроля:** закрепить E2E evidence operation history/recovery screen в CI-сигнале и заменить storefront catalog/cart preview repository на shared-transport backend implementation, не добавляя feature-local transport-клиентов.
...
Следующий storefront mobile шаг: заменить host-owned preview repository на shared-transport backend implementation и подключить storefront packages через generated registry, синхронно с `docs/UI/storefront.md` и web storefront parity rules.
...
Storefront mobile track now has its first module-owned package instead of catalog/cart placeholders:
...
- data boundary — `StorefrontCatalogRepository` is host-provided through Riverpod, so the package does not create a GraphQL client, tenant resolver, auth/session store, or locale fallback chain;
- host route — `apps/rustok_frontend_mobile` mounts the package at `/catalog` and `/cart`, while the host still owns shell navigation and runtime `tenantSlug`/`locale`/GraphQL configuration;
- preview repository — `StorefrontPreviewCatalogRepository` is deliberately host-owned and locale-aware until product/cart backend transport is wired through the shared `app_graphql` layer;
- test evidence — `catalog_screens_test.dart` verifies package UI states with host-provided repositories, and `storefront_router_test.dart` verifies catalog/cart mounting inside the customer storefront shell.

FFA guardrail for this step: this is a mobile client/package boundary only. It adds no Flutter-specific backend API, no feature-local transport client, and no duplicate canonical routing/storage logic.

Next storefront step: replace the preview repository with shared-transport product/cart reads when the canonical GraphQL surface is ready, then move package discovery to generated storefront registry wiring.
