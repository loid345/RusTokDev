...
- Чекпойнт реализации (2026-05-29): сгенерированный Flutter-реестр теперь переносит из `rustok-module.toml` не только маршруты, навигацию и дочерние страницы, но и `locale_namespace`, `permissions` и полные FBA-метаданные `builder_consumer` (`provider_module`, `contract`, `builder_contract_version`, capabilities, degraded modes, toggle profiles). Неполные/deferred consumer-черновики не попадают в mobile snapshot. Это закрывает контрактно-безопасную часть Phase 2 для вложенных маршрутов и surface metadata, но реальные route guards для page builder остаются заблокированы до закрытия backend `PB-FBA-1A`/`PB-FBA-1B`.
