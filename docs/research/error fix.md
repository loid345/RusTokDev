...
- Ранний Wave 5 guardrail slice временно закреплял transitional commerce GraphQL adapter как read-only compatibility fallback; этот промежуточный статус теперь superseded: последующие cleanup slices удалили adapter, `src/transport.rs`, `leptos-graphql`, token/tenant fallback inputs и README removal criteria, а boundary/verifier checks запрещают возврат GraphQL fallback markers.
- Ранний Wave 5 docs-sync slice синхронизировал центральный FFA/FBA readiness board с тогдашним transitional adapter/removal criteria state; текущий registry evidence уже отражает native-only read/write boundary и removed GraphQL fallback.
- Ранний Wave 5 ops-sync slice обновил `docs/modules/implementation-plans-registry.md` с checkpoint для native read/write split и verification gates; текущий актуальный статус ведётся в `crates/rustok-inventory/docs/implementation-plan.md` как `wave5_verification_evidence`.
