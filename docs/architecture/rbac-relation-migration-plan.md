...
- [~] **Фаза 4 — Миграция данных и защитные инварианты (в работе):**
  ...
  - Добавлен helper-скрипт `scripts/rbac_relation_staging.sh` для staged rehearsal (pre-report, dry-run, optional apply, optional rollback, markdown-отчёт + таймстемпированные логи в `artifacts/rbac-staging`); helper формирует machine-readable JSON-инварианты через `cleanup target=rbac-report output=<file>` (pre/post apply/post rollback), rollback-шаги требуют валидный snapshot (`--run-apply` в том же запуске или `--rollback-source=<file>`).
