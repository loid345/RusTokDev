...
- Current phase: admin FFA form/command/view-model hardening + FBA readiness hardening gate
- Last checkpoint: FFA-срез `rustok-commerce-admin` продолжил root/core separation: crate root только wires modules и re-export `ui::CommerceAdmin`; Leptos adapter готовит shipping-profile drafts/form state, cart-promotion/order-change commands, optional filters, badge classes и resolution summary cards через framework-agnostic `admin/src/core.rs`; transport facade остаётся единственной точкой вызова raw `api::*` для covered admin flows.
