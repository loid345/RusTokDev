...
- Current phase: admin FFA core module split + FBA readiness hardening gate
- Last checkpoint: FFA-срез `rustok-commerce-admin` перевёл разросшийся `admin/src/core.rs` в `admin/src/core/` submodules (`common`, `shipping_profile`, `promotion`, `presentation`, `order_change`) согласно FFA rule для нескольких поддоменов; crate root по-прежнему только wires modules и re-export `ui::CommerceAdmin`, а Leptos adapter ходит в framework-agnostic core facade и transport facade.
...
- Evidence:
  ...
  - FFA admin core module split: `admin/src/lib.rs` больше не содержит Leptos render/business code и только wires modules + re-export `CommerceAdmin`; `admin/src/core/mod.rs` реэкспортирует subdomain files для shipping-profile form state/draft preparation, optional filter normalization, cart-promotion command preparation, order-change action draft preparation, lifecycle badge classes, summary/display fallback policy, cart-adjustment view-models и resolution summary mapping;
