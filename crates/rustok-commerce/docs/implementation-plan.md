...
- Last checkpoint: FFA-срез storefront aggregate checkout surface перевёл growing `storefront/src/core.rs` в `storefront/src/core/` с submodules `presentation` и `requests`: cart summary/context/shell view-model policy отделены от route/request/command construction, transport остаётся split на `native_server_adapter`/`graphql_adapter`, а raw native/GraphQL `api::*` calls остаются внутри transport adapters.
...
- Evidence:
  ...
  - FFA storefront transport/core split: aggregate checkout route теперь строит `FetchCommerceRequest`, `CartCommandRequest`, `SelectShippingOptionRequest`, context fallback view-model (`tenant/channel/resolution`) и cart summary view-model (money/count/selected-shipping fallback) в Leptos-free `storefront/src/core/` submodules (`requests`, `presentation`); `storefront/src/transport/mod.rs` owns единственную native-first + GraphQL fallback policy, duplicate combined fallback helpers удалены из `storefront/src/api.rs`, а `native_server_adapter.rs` / `graphql_adapter.rs` являются единственными местами storefront UI package, где вызываются raw native/GraphQL `api::*` functions;
