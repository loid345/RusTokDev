...
- Last checkpoint: Order storefront FFA slice now owns complete-checkout command normalization in `storefront/src/transport.rs`; `rustok-commerce-storefront` builds checkout completion commands through the order-owned transport request before invoking the temporary commerce orchestration adapter.
