...
- Evidence:
  ...
  - unit tests покрывают locale fallback, tags extraction, price sale mapping, search normalization, variant title fallback в backend read-side service, service-level non-negative reservation/availability request invariants, no-create reservation release error semantics и tracked reservation item release guardrail;
