...
fn test_background_trace() {
    ...
    let (html, css) = config
        .compile_html(include_str!("background.html"), &mut builder)
        .unwrap();
    std::fs::write(
        "tests/html/background/background.traced.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/background/background.traced.css",
        css.as_bytes(),
    )
    .unwrap();
...
fn test_background_inline() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/background/background.inline.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/background/background.inline.css",
        css.as_bytes(),
    )
    .unwrap();
