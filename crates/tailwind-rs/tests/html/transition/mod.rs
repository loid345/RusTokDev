...
fn test_transition_trace() {
    ...
    let (html, css) = config
        .compile_html(include_str!("transition.html"), &mut builder)
        .unwrap();
    std::fs::write(
        "tests/html/transition/transition.traced.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/transition/transition.traced.css",
        css.as_bytes(),
    )
    .unwrap();
...
fn test_transition_inline() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/transition/transition.inline.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/transition/transition.inline.css",
        css.as_bytes(),
    )
    .unwrap();
