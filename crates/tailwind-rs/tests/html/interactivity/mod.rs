...
fn test_interactivity_trace() {
    ...
    let (html, css) = config
        .compile_html(include_str!("interactivity.html"), &mut builder)
        .unwrap();
    std::fs::write(
        "tests/html/interactivity/interactivity.traced.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/interactivity/interactivity.traced.css",
        css.as_bytes(),
    )
    .unwrap();
...
fn test_interactivity_inline() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/interactivity/interactivity.inline.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/interactivity/interactivity.inline.css",
        css.as_bytes(),
    )
    .unwrap();
