...
fn test_arbitrary_trace() {
    ...
    let (html, css) = config
        .compile_html(include_str!("arbitrary.html"), &mut builder)
        .unwrap();
    std::fs::write(
        "tests/html/arbitrary/arbitrary.traced.html",
        html.as_bytes(),
    )
    .unwrap();
...
fn test_arbitrary_inline() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/arbitrary/arbitrary.inline.html",
        html.as_bytes(),
    )
    .unwrap();
...
fn test_arbitrary_scoped() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/arbitrary/arbitrary.scoped.html",
        html.as_bytes(),
    )
    .unwrap();
