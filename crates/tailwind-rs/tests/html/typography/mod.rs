...
fn test_typography_trace() {
    ...
    let (html, css) = config
        .compile_html(include_str!("typography.html"), &mut builder)
        .unwrap();
    std::fs::write(
        "tests/html/typography/typography.traced.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/typography/typography.traced.css",
        css.as_bytes(),
    )
    .unwrap();
...
fn test_typography_inline() {
    ...
    let (html, css) = config
        ...
        .unwrap();
    std::fs::write(
        "tests/html/typography/typography.inline.html",
        html.as_bytes(),
    )
    .unwrap();
    std::fs::write(
        "tests/html/typography/typography.inline.css",
        css.as_bytes(),
    )
    .unwrap();
