...
- Current phase: ffa_admin_table_row_view_core_boundary
- Last checkpoint: FFA slice #79 moved blog admin table-row view-model composition into Leptos-free `admin/src/core.rs` (`BlogPostAdminTableRowViewModel`), so the Leptos table fragment consumes prepared display/action state while preserving the existing native/GraphQL transport contract.
...
- [x] Pass #1 (code/docs consistency): storefront helper extraction, `slug` route/query key contract, explicit native/GraphQL adapter split, core-owned fetch request, admin save command, admin editor form-state mapping and admin table-row view-model mapping reflected in tracker and local docs.
