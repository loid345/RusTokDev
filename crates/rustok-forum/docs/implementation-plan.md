...
- Last checkpoint: Forum admin FFA slice дополнительно перенёс tag-chip parsing, numeric position parsing, sidebar active-class policy и status badge class policy в framework-agnostic `admin/src/core.rs`: Leptos adapter больше не держит эти presentation decisions локально, а только применяет core-owned values к markup/signals. REST transport и storefront native/GraphQL fallback contracts не менялись.
- Next step: Продолжать сужать admin `ui/leptos.rs`: переносить оставшиеся page/sidebar/form title envelopes, placeholder policy и SEO panel copy mapping в `admin/src/core.rs`, оставляя REST/GraphQL contracts без изменений.
