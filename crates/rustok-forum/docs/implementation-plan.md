...
- Last checkpoint: Forum admin FFA slice перенёс collection empty/ready/error classification в framework-agnostic `admin/src/core.rs`: category grid/sidebar, topic feed and reply stack now receive core-owned `ForumAdminCollectionState` while Leptos keeps only DOM rendering for each state. REST transport и storefront native/GraphQL fallback contracts не менялись.
- Next step: Продолжать сужать admin `ui/leptos.rs`: переносить оставшиеся form option/list presentation envelopes или topic filter summary policy в `admin/src/core.rs`, оставляя REST/GraphQL contracts без изменений.
