...
- Last checkpoint: Forum admin FFA slice перенёс page-level header selection и loaded-result metric counting в framework-agnostic `admin/src/core.rs`: Leptos adapter получает готовую header view-model policy и вызывает общий count helper вместо owning per-resource loading/error fallback logic. REST transport и storefront native/GraphQL fallback contracts не менялись.
- Next step: Продолжать сужать admin `ui/leptos.rs`: переносить оставшиеся empty-state presentation envelopes или route/query intents в `admin/src/core.rs`, оставляя REST/GraphQL contracts без изменений.
