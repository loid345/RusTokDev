...
- Last checkpoint: Forum admin FFA slice перенёс topic filter summary policy в framework-agnostic `admin/src/core.rs`: выбранная category label теперь вычисляется core helper-ом из loaded categories, selected id, all/fallback labels, а Leptos adapter только привязывает Memo к готовой строке. REST transport и storefront native/GraphQL fallback contracts не менялись.
- Next step: Продолжать сужать admin `ui/leptos.rs`: переносить оставшиеся form option/list presentation envelopes в `admin/src/core.rs`, оставляя REST/GraphQL contracts без изменений.
