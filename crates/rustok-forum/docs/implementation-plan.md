...
- Last checkpoint: Forum admin FFA slice перенёс busy-key, error-message и topic form/sidebar presentation and delete-outcome policy в framework-agnostic `admin/src/core.rs`: category/topic edit/save/delete busy state собирается typed helper-ом, validation/transport error text, category select options, tag-count summary, editing-thread label and delete success clear/refresh/query intents проходят через core helpers, а Leptos adapter только применяет готовые строки/option view-models к signals и markup. REST transport и storefront native/GraphQL fallback contracts не менялись.
