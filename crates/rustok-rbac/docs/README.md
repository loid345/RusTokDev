...
- `casbin_shadow_evaluator` exports `evaluate_casbin_shadow`, `compare_casbin_shadow_decision`, and `permissions_for_shadow_check` for matcher-compatible in-module Casbin shadow checks and unified mismatch payload generation (`single|any|all`) without changing active decision path; generic per-permission traversal is provided by `ShadowCheck::for_each_permission`.
