...
- First mutation-backed modules operator action via the canonical `toggleModule` GraphQL mutation, gated by GraphQL-hydrated `me.permissions` capability context, with post-hook recovery feedback.
...
4. Expand the modules pilot from recovery feedback to retry/compensation actions for lifecycle post-hook failures.
