...
if [[ "$RUSTOK_MIGRATION_SMOKE_INCREMENTAL" != "0" && "$RUSTOK_MIGRATION_SMOKE_INCREMENTAL" != "1" ]]; then
  echo "RUSTOK_MIGRATION_SMOKE_INCREMENTAL must be 0 or 1" >&2
  exit 2
fi

if [[ "$RUSTOK_MIGRATION_SMOKE_KEEP_DB" != "0" && "$RUSTOK_MIGRATION_SMOKE_KEEP_DB" != "1" ]]; then
  echo "RUSTOK_MIGRATION_SMOKE_KEEP_DB must be 0 or 1" >&2
  exit 2
fi
...
echo "Running PostgreSQL migration smoke ($mode) against temporary database '$RUSTOK_MIGRATION_SMOKE_DB_NAME'"
(
  cd "$ROOT_DIR"
