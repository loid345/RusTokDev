...
- Recovery для failed module lifecycle post-hook операций остаётся host/control-plane сценарием: Leptos admin должен читать `moduleOperationRecoveryPlan` / `failedModuleOperationRecoveryPlans` и вызывать `retryFailedModuleOperationPostHook` / `compensateFailedModuleOperation` через canonical GraphQL/server backend, а не выполнять локальный SQL, локальный rollback или собственную lifecycle taxonomy.
