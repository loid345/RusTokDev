...
- RBAC phase-4 staged execution helper: `scripts/rbac_relation_staging.sh` orchestrates `rbac-report -> rbac-backfill(dry-run) -> backfill(apply optional) -> rollback(dry/apply optional)` with timestamped logs + markdown report artifact; helper now also requests machine-readable RBAC invariant snapshots via `cleanup target=rbac-report output=<json>` (pre-check and optional post-check reports in `artifacts/rbac-staging`), appends invariant deltas (`pre -> post`) to the markdown report, and supports `--fail-on-regression` for stop-the-line guardrails when post-check metrics regress; rollback steps require an existing snapshot (`--run-apply` in the same run or `--rollback-source=<file>`), which prevents false rollback attempts without source data; use this for staged dry-run/backfill/rollback rehearsals in `staging` before production cutover.
- RBAC phase-4 helper smoke tests: `scripts/tests/rbac_relation_staging_test.sh` validates fail-fast rollback guards and snapshot-source flows via mocked `cargo` execution (`RUSTOK_CARGO_BIN`).
