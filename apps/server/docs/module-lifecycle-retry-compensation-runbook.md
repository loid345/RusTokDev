...
3. **Повтор toggle с тем же target-state допускается как retry-механизм**, если hook реализован идемпотентно.
4. **Compensation выполняется отдельной осознанной операцией** (новый toggle в обратную сторону), а не скрытым rollback внутри failed post-hook path.
...
2. Выполните повтор toggle на **тот же** target state (например, `enable=true`, если failed был после enable).
3. Проверьте, что создана новая operation-запись со статусом `committed` (или `failed`, если проблема повторилась).

Ожидаемый результат: success retry **не должен** создавать duplicate side effects, а журнал должен показать новый attempt с новым `correlation_id`.
...
2. Выполнить toggle в обратное состояние (новая lifecycle operation).
3. Убедиться, что зависимые модули/policy-инварианты не нарушены перед compensating toggle.
4. Проверить новый `module_operations` trail (failed/success) и текущее состояние `tenant_modules`.

## Минимальный post-incident checklist

- [ ] Для каждого failed post-hook случая зафиксирован `correlation_id` и root cause.
- [ ] Retry или compensation выполнены через canonical lifecycle entrypoint (не через bypass/SQL).
